...
namespace Binder {
	...
	constexpr unsigned int get_map_value(char const* a){
		...
		if (strings_equal("geometry_vertex.glsl", a)) { return 2; }
		if (strings_equal("lightsource_vertex.glsl", a)) { return 3; }
		if (strings_equal("shadow_cube_vertex.glsl", a)) { return 4; }
		if (strings_equal("shadow_vertex.glsl", a)) { return 5; }
	};
	...
	struct type_list
{
	...
	struct AttributeObject {
		template<char const* str>
		using Get = typename type_list<AttributeStructure_deferred_vertex,AttributeStructure_depth_map_vertex,AttributeStructure_geometry_vertex,AttributeStructure_lightsource_vertex,AttributeStructure_shadow_cube_vertex,AttributeStructure_shadow_vertex>::type<str>;
	};

	namespace file_names{
		constexpr char deferred_frag[] = "deferred_frag.glsl";
		constexpr char deferred_vertex[] = "deferred_vertex.glsl";
		constexpr char depth_map_frag[] = "depth_map_frag.glsl";
		constexpr char depth_map_vertex[] = "depth_map_vertex.glsl";
